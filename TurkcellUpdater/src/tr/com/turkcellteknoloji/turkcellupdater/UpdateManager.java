...
public class UpdateManager {
	 ...
	 * Conditionally one of 4 methods will be called at the end of a update
	 * check:
	 * <ul>
	 * <li>A new version is found,
	 * {@link UpdateCheckListener#onUpdateCheckCompleted(UpdateManager, Update)}
	 * </li>
	 * <li>A message should be displayed to user,
	 * {@link UpdateCheckListener#onUpdateCheckCompleted(UpdateManager, Message)}
	 * </li>
	 * <li>Completed successfully and no messages and no new versions found,
	 * {@link UpdateCheckListener#onUpdateCheckCompleted(UpdateManager)}</li>
	 * <li>Update check is fail due to an error</li>
	 * </ul>
	 * 
	 * @see UpdateManager#checkUpdates(Context, URI, Properties,
	 *      UpdateCheckListener)
	 * @author Ugur Ozmen
	 */
	public interface UpdateCheckListener {
		/**
		 * This method is called when update check is completed successfully and
		 * a newer version is found. Implementations should display
		 * {@link Update#description} to users. Implementations should not
		 * provide an option to cancel and continue to application if
		 * {@link Update#forceUpdate} is true.
		 * 
		 * @param manager
		 *            Update manager that has performed update check.
		 * @param update
		 *            Information about next version found.
		 */
		void onUpdateCheckCompleted(UpdateManager manager, Update update);

		/**
		 * This method is called when update check is completed successfully and
		 * a message should be displayed to user. Implementations should not
		 * continue normal operation after this message is dismissed.
		 * 
		 * @param manager
		 *            Update manager that has performed update check.
		 * @param message
		 *            Information delivered from server that should be displayed
		 *            to user.
		 */
	...
	public interface UpdateListener {
		 ...
		 * 
		 * @param percent
		 *            <code>null</code> if current state is interminable
		 *            otherwise percent of completed progress
		 */
		void onUpdateProgress(Integer percent);

		/**
		 * Called when update is cancelled. Implementations should close
		 * application if {@link Update#forceUpdate} is true.
		 */
		void onUpdateCancelled();

		/**
		 * Called when update is successfully completed. Implementations should
		 * close application if {@link Update#forceUpdate} is true.
		 */
	...
	/**
	 * Starts an asynchronous operation for checking if a newer version of
	 * current application is available.
	 * 
	 * @param context
	 *            Current context.
	 * @param versionServerUri
	 *            Location of update definitions.
	 * @param currentProperties
	 *            properties of current application and device.
	 * @param postProperties
	 *            <code>true</code> if current properties should post to server
	 *            for server side processing.
	 * @param listener
	 *            Callback listener.
	 * @throws UpdaterException
	 *             if operation couldn't be started.
	 */
	public void checkUpdates(Context context, final URI versionServerUri,
			Properties currentProperties, boolean postProperties,
			UpdateCheckListener listener) throws UpdaterException {
		final DefaultHttpClient client = Utilities
				.createClient("TurkcellUpdater/1.0");
		final VersionMapRequest request = new VersionMapRequest(context,
				versionServerUri, currentProperties, postProperties, listener);
		try {
	...
	/**
	 * Starts an asynchronous operation for installing newer version of
	 * application.
	 * 
	 * @param context
	 *            Current context.
	 * @param update
	 *            Information about next version.
	 * @param listener
	 *            Callback listener.
	 */
	public void applyUpdate(Context context, Update update,
		...
		if (update.targetGooglePlay) {
			...
			try {
				...
				dr.setDownloadHandler(new DownloadHandlerAdapter(context,
						listener));

				HttpClient client = Utilities
						.createClient("Turkcell Updater/1.0 ");
	...
	private void installPackage(final Context context,
			final byte[] apkContents, final UpdateListener listener) {
			...
			protected File doInBackground(Void... params) {
				try {
					...
					FileOutputStream fout = context.openFileOutput(
							tempFileName, Context.MODE_WORLD_READABLE);
					try {
	...
	private void openGooglePlayPage(Context context, String packageName,
			final UpdateListener listener) {
		if (listener != null) {
	...
	private void openWebPage(Context context, URL url,
			final UpdateListener listener) {
		if (listener != null) {
			listener.onUpdateProgress(null);
		}
	...
	private class VersionMapRequest extends RestRequest {
		...
		public VersionMapRequest(final Context context,
				final URI versionServerUri, final Properties currentProperties,
				boolean postProperties, final UpdateCheckListener listener) {
			...
			setResultHandler(new RestJsonObjectResultHandler() {
				...
				public void onSuccess(JSONObject jsonObject) {
					try {
						final String packageName = currentProperties
								.getValue(Properties.KEY_APP_PACKAGE_NAME);
						if (jsonObject != null
						...
						if (VersionsMap.isVersionMapOfPackageId(packageName,
								jsonObject)) {
							VersionsMap map = new VersionsMap(jsonObject);
							final Update update = map
									.getUpdate(currentProperties);
							if (update != null) {
								Log.i("Update found: " + update);
								listener.onUpdateCheckCompleted(
										UpdateManager.this, update);
							} else {
								final MessageDisplayRecords records = new MessageDisplayRecords(
										context);
								final Message message = map.getMessage(
										currentProperties, records, context);
								if (message == null) {
									...
									listener.onUpdateCheckCompleted(
											UpdateManager.this, message);
								}
							}
						} else {
							throw new UpdaterException(
									"Configuration file packagename should be: "
											+ packageName);
						}
					} catch (Exception e) {
