...
class MessageEntry extends FilteredEntry {
    ...
    MessageEntry(List<Filter> filters, int id, List<MessageDescription> messageDescriptions, int displayPeriodInHours, Date displayAfterDate, Date displayBeforeDate, int maxDisplayCount, String targetPackageName, URL targetWebsiteUrl, boolean targetGooglePlay) throws UpdaterException {
        super(filters);
    ...
    private int generateId() {
        ...
        result = prime * result + ((messageDescriptions == null) ? 0 : messageDescriptions.hashCode());
        return result;
    }
    ...
    private static List<MessageDescription> createMessageDescriptions(JSONObject jsonObject) {
        ...
        if (udsObject != null) {
            ...
            while (languages.hasNext()) {
                ...
                if (o != null) {
                    MessageDescription ud = new MessageDescription(languageCode, o);
                    result.add(ud);
                } else {
                    final String s = udsObject.optString(languageCode, null);
                    if (s != null) {
                        MessageDescription ud = new MessageDescription(languageCode, s);
                        result.add(ud);
                    }
                }
            }
